...
export const Hero = () => {
  return (
    <section className="relative w-full max-w-7xl mx-auto py-20 px-6 flex flex-col md:flex-row items-center justify-between bg-[var(--background)]">
      {/* Texte */}
      <div className="w-full lg:w-1/2 space-y-6 text-center lg:text-left font-sans relative z-10">
        <h1 className="text-2xl lg:text-4xl font-bold text-brand-dark text-justify">
        ...
        <p className="text-lg text-brand-dark font-sans text-justify">
      ...
      <div className="w-full lg:w-[40%] mt-12 lg:mt-0 flex flex-row md:flex-col ipadPro:flex-row justify-between items-center justify-center relative z-10">
        {/* Bloc flottant des icônes */}
        <div className="flex flex-col md:flex-row ipadPro:flex-col items-center justify-center gap-10 w-[200px]">
          {/* Idée */}
          <motion.div
            animate={{ x: ["0%", "80%", "0%", "-80%", "0%"] }}
            transition={{
              duration: 6,
              repeat: Infinity,
              ease: "easeInOut",
